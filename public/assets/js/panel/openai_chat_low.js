...
$(document).ready(function () {
	...
	$('.chats-container')
		.stop()
		.animate({ scrollTop: $('.chats-container')[0]?.scrollHeight }, 200);
	$('#scrollable_content')
		.stop()
...
function makeDocumentReadyAgain() {
	...
	$(document).ready(function () {
		...
		$('.chats-container')
			.stop()
			.animate(
				{ scrollTop: $('.chats-container')[0]?.scrollHeight },
				200
			);
		$('#scrollable_content')
			.stop()
			.animate(
				{ scrollTop: $('#scrollable_content').outerHeight() },
				200
			);
	...
	setTimeout(function () {
		...
		const generate = async ev => {
			...
			if (category.slug == 'ai_chat_image') {
				...
				jQuery('.chats-container')
					.stop()
					.animate(
						{
							scrollTop:
								jQuery('.chats-container')[0]?.scrollHeight,
						},
						200
					);
				jQuery('#scrollable_content')
					.stop()
					.animate(
						{
							scrollTop: jQuery(
			...
			function onWindowScroll() {
				if (
					chatsContainer[0].scrollTop +
					chatsContainer[0].offsetHeight >=
					chatsContainer[0].scrollHeight
				) {
					scrollLocked = true;
				} else {
					scrollLocked = false;
			...
			nIntervId = setInterval(function () {
				if (chunk.length == 0 && !streaming) {
					...
					chatsContainer[0].removeEventListener(
						'scroll',
						onWindowScroll
					);
		...
		success: function (data) {
			...
			setTimeout(function () {
				$('.chats-container')
					.stop()
					.animate(
						{ scrollTop: $('.chats-container').outerHeight() },
						200
					);
			}, 750);
		},
