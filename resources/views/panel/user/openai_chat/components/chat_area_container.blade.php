    ...
    <div class="flex gap-2">
        ...
        <div class="self-center lqd-chat-mobile-sidebar-trigger">
            <button class="w-8 h-8 grid-flow-row p-0 btn group place-items-center sm:hidden max-sm:grid"
                type="button" data-lqd-toggle-class="lqd-is-active" data-lqd-toggle-target="#chats-list-container">
...
<div class="conversation-area flex grow flex-col justify-between overflow-y-auto max-md:max-h-[70vh] max-sm:max-h-full"
    ...
    @else --}}
    <div class="relative flex flex-col grow max-sm:max-h-full max-sm:overflow-y-auto">
        <div
    ...
    <form
        ...
        <div class="form-control flex min-h-[52px] rounded-[26px] p-0 max-sm:min-h-[45px]">
            ...
            <div class="relative flex items-start grow">
                ...
                <button
                    class="lqd-chat-mobile-options-trigger btn text-heading collapsed h-8 w-8 shrink-0 origin-center bg-transparent p-0 mt-[7px] shadow-none transition-transform sm:hidden [&:not(.collapsed)]:rotate-45"
                ...
                <textarea
                    class="text-heading m-0 w-full border-none bg-transparent !py-3 pe-[100px] ps-16 outline-none focus:border-none focus:ring-0 max-sm:pe-2 max-sm:ps-0 max-sm:text-sm max-sm:max-h-[200px]"
                        ...
                        <div class="flex items-center pointer-events-auto max-sm:pt-4">
                            <button type="button"
                                @if (env('APP_STATUS') == 'Demo' &&
                                        (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf'))) onclick="return toastr.info('This feature is disabled in Demo version.')" @else id="chat_add_image" @endif
        ...
        @if ($setting->hosting_type == 'high')
            <button
                ...
                @if (env('APP_STATUS') == 'Demo' && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf')) @else id="send_message_button" @endif form="chat_form">
        ...
        @else
            ...
            <button
                @if (env('APP_STATUS') == 'Demo' && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf')) onclick="return toastr.info('This feature is disabled in Demo version.')" @endif
                class="btn btn-primary h-[52px] w-[52px] shrink-0 rounded-full p-0 max-sm:h-10 max-sm:w-10"
                @if (env('APP_STATUS') == 'Demo' && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf')) @else id="send_message_button" @endif type="button">
                <svg class="rtl:-scale-x-100" width="16" height="14" viewBox="0 0 16 14" fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg">
                    <path d="M0.125 14V8.76172L11.375 7.25L0.125 5.73828V0.5L15.875 7.25L0.125 14Z" />
                </svg>
            </button>
            <button class="btn btn-primary hidden h-[52px] w-[52px] shrink-0 rounded-full" id="stop_button"
                type="button">
