<div id="chats-list-container"
    class="chats-list-container flex w-1/4 shrink-0 grow-0 flex-col overflow-hidden border-b-0 border-l-0 border-t-0 border-solid border-[--tblr-border-color] sm:!flex max-md:h-[50vh] max-md:w-full max-sm:h-0 max-sm:overflow-hidden max-sm:absolute max-sm:start-0 max-sm:transition-all max-sm:duration-300 max-sm:border-none max-sm:top-[82px] max-sm:z-20 max-sm:bg-[--tblr-body-bg] [&.lqd-is-active]:h-[calc(100%-82px)]">
    <div
        class="chats-search border-l-0 border-r-0 border-t-0 border-solid border-[--tblr-border-color] p-[20px] max-xl:p-[10px]">
    ...
    <div class="px-6 mt-auto chats-new py-11 max-xl:p-4">
        <a href="javascript:void(0);"
            @if (env('APP_STATUS') == 'Demo' &&
                    (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf'))) onclick="return toastr.info('This feature is disabled in Demo
			version.')" @else onclick="return startNewChat({{ $category->id }}, '{{ LaravelLocalization::getCurrentLocale() }}');" @endif
