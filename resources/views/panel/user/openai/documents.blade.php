            ...
            <div class="row g-2 items-end justify-between max-md:flex-col max-md:items-start max-md:gap-4">
						...
						<div class="flex flex-wrap items-center">
							...
							<div class="grow-0 shrink-0 relative !me-1">
								...
								<div class="dropdown-menu z-10">
									<button data-sort-type="file" class="flex items-center gap-2 w-full p-2 px-3 border-none rounded-md bg-[transparent] text-[12px] !no-underline font-medium text-heading hover:bg-slate-100 dark:hover:bg-zinc-900">
										{{__('Type')}}
									</button>
                                    <button data-sort-type="name" class="flex items-center gap-2 w-full p-2 px-3 border-none rounded-md bg-[transparent] text-[12px] !no-underline font-medium text-heading hover:bg-slate-100 dark:hover:bg-zinc-900">
										{{__('Name')}}
									</button>
									<button data-sort-type="date" class="flex items-center gap-2 w-full p-2 px-3 border-none rounded-md bg-[transparent] text-[12px] !no-underline font-medium text-heading hover:bg-slate-100 dark:hover:bg-zinc-900">
										{{__('Date')}}
									</button>
                                    <button data-sort-type="cost" class="flex items-center gap-2 w-full p-2 px-3 border-none rounded-md bg-[transparent] text-[12px] !no-underline font-medium text-heading hover:bg-slate-100 dark:hover:bg-zinc-900">
										{{__('Cost')}}
									</button>
								</div>
							</div>
                            <form id="filterForm" method="GET" action="{{ route('dashboard.user.openai.documents.all') }}">
                                ...
                                <ul class="flex flex-wrap items-center m-0 p-0 !ms-2 list-none text-[13px] text-[#2B2F37] gap-[20px] max-sm:gap-[10px]">
                                    <li>
                                        <button name="filter" value="all" data-filter-trigger="all" class="filter-button inline-flex leading-none p-[0.3em_0.65em] rounded-full bg-[transparent] border-0 text-inherit hover:no-underline hover:bg-black/5 transition-colors [&.active]:bg-black/5 dark:text-[--tblr-muted] dark:[&.active]:bg-[--lqd-faded-out] dark:[&.active]:text-[--lqd-heading-color] dark:hover:bg-white/5 {{$filter=="all"?"active":""}}">
                                            {{__('All')}}
                                        </button>
                                    </li>
                                    <li>
                                        <button name="filter" value="favorites" data-filter-trigger="favorite" class="filter-button inline-flex leading-none p-[0.3em_0.65em] rounded-full bg-[transparent] border-0 text-inherit hover:no-underline hover:bg-black/5 transition-colors [&.active]:bg-black/5 dark:text-[--tblr-muted] dark:[&.active]:bg-[--lqd-faded-out] dark:[&.active]:text-[--lqd-heading-color] dark:hover:bg-white/5 {{$filter=="favorites"?"active":""}}">
                                            {{__('Favorites')}}
                                        </button>
                                    </li>
                                    <li>
                                        <button name="filter" value="text" data-filter-trigger="text" class="filter-button inline-flex leading-none p-[0.3em_0.65em] rounded-full bg-[transparent] border-0 text-inherit hover:no-underline hover:bg-black/5 transition-colors [&.active]:bg-black/5 dark:text-[--tblr-muted] dark:[&.active]:bg-[--lqd-faded-out] dark:[&.active]:text-[--lqd-heading-color] dark:hover:bg-white/5 {{$filter=="text"?"active":""}}">
                                            {{__('Text')}}
                                        </button>
                                    </li>
                                    <li>
                                        <a href="{{route('dashboard.user.openai.generator', 'ai_image_generator')}}" data-filter-trigger="image" class="filter-button inline-flex leading-none p-[0.3em_0.65em] rounded-full bg-[transparent] border-0 text-inherit hover:no-underline hover:bg-black/5 transition-colors [&.active]:bg-black/5 dark:text-[--tblr-muted] dark:[&.active]:bg-[--lqd-faded-out] dark:[&.active]:text-[--lqd-heading-color] dark:hover:bg-white/5 {{$filter=="image"?"active":""}}">
                                            {{__('Image')}}
                                        </a>
                                    </li>
                                    <li>
                                        <button name="filter" value="code" data-filter-trigger="code" class="filter-button inline-flex leading-none p-[0.3em_0.65em] rounded-full bg-[transparent] border-0 text-inherit hover:no-underline hover:bg-black/5 transition-colors [&.active]:bg-black/5 dark:text-[--tblr-muted] dark:[&.active]:bg-[--lqd-faded-out] dark:[&.active]:text-[--lqd-heading-color] dark:hover:bg-white/5 {{$filter=="code"?"active":""}}">
                                            {{__('Code')}}
                                        </button>
                                    </li>
                                    {{-- <li>
                                        <button name="filter" value="transcription" data-filter-trigger="transcription" class="filter-button inline-flex leading-none p-[0.3em_0.65em] rounded-full bg-[transparent] border-0 text-inherit hover:no-underline hover:bg-black/5 transition-colors [&.active]:bg-black/5 dark:text-[--tblr-muted] dark:[&.active]:bg-[--lqd-faded-out] dark:[&.active]:text-[--lqd-heading-color] dark:hover:bg-white/5 {{$filter=="transcription"?"active":""}}">
                                            {{__('Transcription')}}
                                        </button>
                                    </li> --}}
                                </ul>
				...
				<div class="col-auto">
					<div class="btn-list">
                        ...
                        <a @if(env('APP_STATUS') == 'Demo')  onclick="return toastr.info('This feature is disabled in Demo version.')" @else data-bs-toggle="modal" data-bs-target="#creatFolderModal" @endif class="btn btn-primary items-center">
					...
					<ul class="flex items-center list-none m-0 mt-3 p-0 gap-2 lg:justify-end">
						<li>
							<button class="inline-flex w-8 h-8 items-center bg-transparent border-none justify-center rounded-md text-heading -mb-4 hover:!bg-black/5 dark:hover:!bg-white/5" onclick="toggleView('dlist')" title="List view">
						...
						<li>
							<button class="inline-flex w-8 h-8 items-center bg-transparent border-none justify-center rounded-md text-heading -mb-4 hover:!bg-black/5 dark:hover:!bg-white/5" onclick="toggleView('dgrid')" title="Grid view">
								<svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
